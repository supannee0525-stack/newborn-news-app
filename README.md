...
MAP is calculated automatically from `DBP + ((SBP - DBP) / 3)`, rounded down to a whole mmHg (never rounded up, so a borderline value is never hidden), and is not entered manually.
